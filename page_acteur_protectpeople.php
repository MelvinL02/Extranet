...
if ($_SESSION["id_user"]) {
    $select = $pdo->prepare(
        "SELECT u.id_user FROM users u INNER JOIN likes l ON u.id_user = l.id_membre WHERE l.id_membre = ? AND l.id_article = 2"
    );
}
...
if ($select->rowCount() > 0) {
    ...
    $select = $pdo->prepare(
        "SELECT u.id_user FROM users u INNER JOIN dislikes d ON u.id_user = d.id_membre WHERE d.id_membre = ? AND d.id_article = 2"
    );
    $select->execute([$_SESSION["id_user"]]);
    if ($select->rowCount() > 0) {
        $select = $select->fetch();
		...
		<main>
			<section id="presentation_acteur">
				...
				<div class="description_page_acteur">
					<h2>Protectpeople</h2>
					<p>
						<strong>Protectpeople finance la solidarité nationale.</strong>
					</p>
					<p>
						<strong>Nous appliquons le principe édifié par la Sécurité sociale française en 1945 : permettre à chacun de bénéficier d’une protection sociale.</strong>
					</p>
					<p>
