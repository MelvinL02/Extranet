<?php
  ...
  if(empty($comment)) header("Location: $referer");
   else{
  ...
  if ($select->rowCount() > 8) {
    ...
    $ins = $pdo->prepare("INSERT INTO comments (id_actor,prenom,comment) VALUES (?,?,?)");
    $executeIsOk = $ins->execute(array($actor, $prenom, $comment));
    }
