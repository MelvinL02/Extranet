...
if (isset($valider)) {
    if (empty($nom)) {
        ...
        $erreur = "Veuillez remplir le champ utilisateur !";
    } elseif (empty($question)) {
        $erreur = "Veuillez remplir le champ question !";
    } elseif (empty($reponse)) {
        $erreur = "Veuillez remplir le champ reponse !";
        ...
        $executeIsOk = $stmt->execute([$username]);
        $tab = $stmt->fetchAll();
        if (count($tab) > 0) {
            $erreur = "L'utilisateur existe déjà!";
        } else {
            $ins = $pdo->prepare(
                "INSERT into users(nom,prenom,username,password,question,reponse) values(?,?,?,?,?,?)"
            );
            if (
                $ins->execute([
                    $nom,
